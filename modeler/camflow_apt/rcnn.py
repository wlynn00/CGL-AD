...
def test_v1(model, test_dataloader, args):
    ...
    for g in sorted(results):
        print("{}: max-{}, mean-{}, std-{}".
              format(g, np.max(results[g]), np.mean(results[g]), np.std(results[g])))
    search_list = np.arange(2.0, 4.1, 1)
    # search_list = [5.0]
    pair_result = {}
    for num_std in search_list:
        pred_l, true_l = [], []
        print(f'\n******num_std={num_std}******')
        thre_map = dict()
        for g in sorted(results):
            thre_g = np.mean(results[g]) + num_std * np.std(results[g])
            thre_map[g] = thre_g
            ab_num = 0
            ...
            print(f'{g} thre: {thre_g}, anomaly num: {ab_num}/{len(results[g])}, ab_rate: {ab_rate}')

            y = 1 if g > 124 else 0
            y_hat = 1 if ab_num > 0 else 0
            true_l.append(y)
            pred_l.append(y_hat)
        pair_result[num_std] = [np.array(true_l), np.array(pred_l)]
    return pair_result
...
def test(model, test_dataloader, args):
    ...
    print(group_stats['attack_loss'])
    print(group_stats['benign_loss'])
    ...
    thre_list = np.arange(0.0002, 0.00025, 0.00002)   # threshold to meature anomaly
    pair_result = {}
        ...
        for g in sorted(results):
            ...
            for num in results[g]:
                if num > threshold:
                    ab_num += 1
            ab_rate = ab_num/len(results[g])
            print(f'{g} anomaly num: {ab_num}/{len(results[g])}, rate:{ab_rate}')
...
def main(args):
    ...
    val_list.sort()
    test_list.sort()
    print('val graphs:', val_list)
    print('test graphs:', test_list)
    # Train
    if args.train:
    ...
    if args.test:
        ...
        # test_results = test_v1(model, test_dataloader, args)
        # exit()
...
def get_predict_result(args, data_path, model, model_path, test_list):
    test_seq, test_sna, test_label = load_data(data_path, test_list, args.seq_len)
    print("[INFO]Load test tgn embedding data finished.")
    test_dataset = CustomDataset(test_seq, test_sna, test_label)
    test_dataloader = DataLoader(dataset=test_dataset, batch_size=args.batch_size, shuffle=True)
...
def result_fusion():
    ...
    true_l = []
    pred_l = []
    sketch_abnormal = [124, 125, 126, 127, 128, 129, 130, 131, 132, 133, 134, 135, 136, 137,
                 138, 139, 140, 141, 142, 143, 144, 145, 146, 147, 148, 149]
    print(f"Histosketch+RCNN predict abnormal(cnt:{len(sketch_abnormal)}): {sketch_abnormal}")
    tgn_abnormal = [14, 29, 68, 88, 99, 106, 124, 125, 126, 127, 128, 129, 130, 131, 132, 133, 134, 135, 136, 137,
...
if __name__ == "__main__":
    ...
    # data
    parser.add_argument('-i', "--data_path", type=str, default="../../data/camflow_apt/tgn_embedding")
    # parser.add_argument('-i', "--data_path", type=str, default="../../data/camflow_apt/graph_sketch")
