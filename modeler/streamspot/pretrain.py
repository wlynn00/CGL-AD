...
import torch
from torch.nn import Module, Linear, BCEWithLogitsLoss, CrossEntropyLoss
...
def load_pretrain_data():
    data_dir = "../../data/streamspot/temporal_data"
    graph_5 = torch.load(data_dir + '/normal-5.TemporalData').to(device)
    graph_15 = torch.load(data_dir + '/normal-15.TemporalData').to(device)
    graph_125 = torch.load(data_dir+'/normal-125.TemporalData').to(device)
    graph_150 = torch.load(data_dir+'/normal-150.TemporalData').to(device)
    graph_225 = torch.load(data_dir+'/normal-225.TemporalData').to(device)
    graph_250 = torch.load(data_dir+'/normal-250.TemporalData').to(device)
    graph_425 = torch.load(data_dir + '/normal-425.TemporalData').to(device)
    graph_450 = torch.load(data_dir + '/normal-450.TemporalData').to(device)
    graph_525 = torch.load(data_dir + '/normal-525.TemporalData').to(device)
    graph_550 = torch.load(data_dir + '/normal-550.TemporalData').to(device)

    graph_100 = torch.load(data_dir+'/normal-100.TemporalData').to(device)   # val

    return [graph_5, graph_15, graph_125, graph_150, graph_225, graph_250,
            graph_425, graph_450, graph_525, graph_550], graph_100
...
train_list = [5, 15, 125, 150, 225, 250, 425, 450, 525, 550]
for epoch in tqdm(range(1, EPOCH+1)):
    ...
    for i in range(len(pre_data)):
        ...
        total_loss += loss
        print(f"Epoch:{epoch}, g_{gid} Loss: {loss:.5f}")
    loss_test = test_new(pre_val_data)
    avg_loss = total_loss / len(pre_data)
    print(f'  ***Epoch {epoch:02d}, average loss: {avg_loss:.5f}, val Loss: {loss_test:.5f}')
