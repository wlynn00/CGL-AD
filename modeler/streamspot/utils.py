...
def load_data(dir_path, graph_list, length):
    ...
    for graph in tqdm(graph_list, desc='Loading sketch vectors'):
        g_label = graph
        # filename = 'attack-{}.txt'.format(graph) if graph>=300 and graph<400 else 'normal-{}.txt'.format(graph)
        filename = 'sketch-attack-{}.txt'.format(graph) if graph>=300 and graph<400 else 'sketch-normal-{}.txt'.format(graph)
        file_path = os.path.join(dir_path, filename)
        # data = pd.read_csv(file_path, sep=' ', header=None)   # tgn embedding
        data = pd.read_csv(file_path, sep=' ', header=None)   # sketch vector
        data = data.drop(data.columns[-1], axis=1)  # sketch vector去掉空格
